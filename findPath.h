...
std::vector<int> findPath(ListDigraph& gr,
    ...
    // STOPPING RULE
    if(enumeration.len() > 0 && enumeration[0] == src && enumeration.outArcs(enumeration[0]) == 0) {
        cout << "inside the stopping rule" << endl;
	...
	if(num_paths == 0) {
        ...
        cout << "inside the current node had many outArcs" << endl;
            ...
            // find a new path to the target node
            std::cout << "running sub_dfs..." << std::endl;
            cout << "the current node id is " << gr.id(curr_node) << endl;
            cout << "the target node id is " << gr.id(trg) << endl;
            sub_dfs.run(curr_node, trg);
